...
describe("m4 integrated test", () => {
  ...
  beforeEach(() => {
    ...
    let spiedClient = spy(client);

    // TODO: Create the spyClientInstance as an instance of the client spy
    spyClientInstance = instance(spiedClient);

    // TODO: Use the ts-mokito when/thenReturn to tell the spyClient (not the client) to return the instance of
    //  the mock service when the factory method is called
    when(spiedClient.serviceFactory()).thenReturn(mockServiceInstance);
  });
  ...
  it("testConvertValue", () => {
    ...
    // TODO: Use ts-mockito when/thenReturn to tell the mock service (not the instance of the mock service)
    // to return 2 for input 35 when it's getDecimalDigitCount(int) method is called
  ...
  it("testCreateFormattedStringsWithAnswer", () => {
    ...
    when(mockService.processList(anything())).thenCall(() =>
      verify(mockService.processList(notNull()))
    );
    ...
    verify(mockService.processList(anything()));
  });
});
