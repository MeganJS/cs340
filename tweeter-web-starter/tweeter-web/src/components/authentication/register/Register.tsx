...
const Register = () => {
  ...
  //const presenter = new RegisterPresenter(observer);
  const presenter = useRef<RegisterPresenter | null>(null);
  if (!presenter.current) {
    presenter.current = new RegisterPresenter(observer);
  }

  const registerOnEnter = (event: React.KeyboardEvent<HTMLElement>) => {
    if (event.key == "Enter" && !checkSubmitButtonStatus()) {
      presenter.current!.doRegister(
        firstName,
        lastName,
        alias,
        password,
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    presenter.current!.handleImageFile(file);
  };
